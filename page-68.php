...
//Checks the country before anything else

	if (getenv('HTTP_GEOIP_COUNTRY_CODE') == "GB" && $_COOKIE["GEO"] !== "EU") {
		setcookie("GEO", getenv('HTTP_GEOIP_COUNTRY_CODE'), 0, '/');  /* expire in 1 day*/
		wp_redirect("/uk/");	
		exit();
	} 

	if (getenv('HTTP_GEOIP_COUNTRY_CODE') == "US" && $_COOKIE["GEO"] !== "EU") {
		setcookie("GEO", getenv('HTTP_GEOIP_COUNTRY_CODE'), 0, '/');  /* expire in 1 day*/
		wp_redirect("/usa/");	
		exit();
	}
...
<section class="fund fancy_arrangement">
    <?php
        ...
        if( $posts ): 
            ...
            ?>
            <?php foreach( array_slice($posts, 0, 21) as $post): // variable must be called $post (IMPORTANT) ?>
